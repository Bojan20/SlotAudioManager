...
export default function BuildPage({ project, showToast }) {
  ...
  const [gameScripts, setGameScripts] = useState([]); // launch scripts from game repo
  ...
  const runGameScript = async (scriptName) => {
    ...
    try {
      ...
      setLog(r.output || r.error || 'No output');
      setResult({ script: scriptName, ok: r.success });
      showToast(r.success ? `${scriptName} started` : `${scriptName} failed`, r.success ? 'success' : 'error');
    } catch (e) {
      setLog('Error: ' + e.message);
      setResult({ script: scriptName, ok: false });
  ...
  return (
    <div className="anim-fade-up space-y-5 max-w-4xl">
      ...
      <div className="card p-5 space-y-4">
        ...
        {gameRepoPath && (
          <p className="text-[11px] text-text-dim font-mono truncate">{gameRepoPath}</p>
        )}

        {gameScriptsError && (
          <p className="text-[12px] text-danger font-mono">{gameScriptsError}</p>
        )}

        {!deployTarget ? (
          <p className="text-[12px] text-text-dim">Link a game repo in Setup first</p>
          ...
          <div className="space-y-2">
            {gameScripts.map(({ name, cmd }) => (
              <div key={name} className="flex items-center gap-3 py-1.5">
                <div className="flex-1">
                  <p className="text-[13px] font-mono font-medium">{name}</p>
                  <p className="text-[10px] text-text-dim font-mono truncate mt-0.5">{cmd}</p>
                </div>
                <button
