...
/**
 * AAC encoder adds padding samples at start/end of file. Native <audio>.loop
 * replays including padding → audible click/gap at loop point.
 *
 * Fix: monitor <audio>.currentTime via requestAnimationFrame. When within 50ms
 * of the end, mute the node → seek to 0 → unmute. The mute covers any click
 * from the seek. rAF gives ~16ms precision — more than enough for music.
 *
 * Sprite duration is 24h to prevent Howler's setTimeout from interfering.
 */
const loopMonitors = new Map<string, { active: boolean; duration: number }>();

function startLoopMonitor(howl: Howl, spriteId: string, durationSec: number): void {
    const state = loopMonitors.get(spriteId) || { active: false, duration: durationSec };
    loopMonitors.set(spriteId, state);
    ...
    function tick(): void {
        ...
        if (pos > 0.1 && remaining > 0 && remaining < 0.05) {
            // Mute → seek → unmute: covers any click from AAC padding
            const vol = snd._node.volume;
            snd._node.volume = 0;
            snd._node.currentTime = 0;
            // Restore volume next frame after browser processes seek
            requestAnimationFrame(() => {
                if (snd._node) snd._node.volume = vol;
            });
        }
...
function stopLoopMonitor(spriteId: string): void {
    const state = loopMonitors.get(spriteId);
...
function registerHtml5(name: string, url: string, player: any): Promise<boolean> {
    return new Promise((resolve) => {
        ...
        // Fix playa-core resume: set _playStart so pauseAllSounds can resume
        howl.on("play", () => {
            const snd = (howl as any)._sounds?.find((x: any) => x._sprite === spriteId);
            if (snd) {
...
function syncTagState(player: any, spriteId: string): void {
    ...
    if (!sp?._tags) return;
    for (const tag of sp._tags) {
        const td = player._tags?.get(tag);
        if (td?.muted) { sp._isMuted = true; try { sp.mute(); } catch (_) {} }
    }
}
...
function replayMissedCommands(player: any): void {
    ...
    const bootCmds = ["onGameInit", "onBaseGameStart", "onGameStart"];

    for (const cmdName of bootCmds) {
        ...
        for (const step of arr) {
            ...
            sp.play();
            for (const fs of arr) {
                if (!fs || (fs.command || "").toLowerCase() !== "fade") continue;
                if (fs.spriteId !== sid || !(fs.volume > 0)) continue;
                setTimeout(() => {
                    const c = player._soundSprites?.get(sid);
                    if (c?._isPlaying) c.fade({ volume: fs.volume, duration: fs.duration || 1500 });
                }, fs.delay || 50);
                break;
            }
...
/**
 * Save position on hide, restore on show. Quick fade to avoid pop.
 * playa-core's own pauseAllSounds now works (we fixed _playStart above),
 * but we still handle position save/restore and fade as safety net.
 */
function setupVisibilityHandler(player: any): void {
    const musicIds = MUSIC.map(n => "s_" + n);
    const saved = new Map<string, { vol: number; pos: number }>();
    ...
    document.addEventListener("visibilitychange", () => {
        if (document.hidden) {
            ...
            for (const sid of musicIds) {
                ...
                saved.set(sid, { vol: sp._volume, pos: snd?._node?.currentTime || 0 });
                // Quick mute — playa-core will pause via pauseAllSounds
                if (snd?._node) snd._node.volume = 0;
            }
        } else {
            for (const [sid, state] of saved) {
                const sp = player._soundSprites?.get(sid);
                if (!sp) continue;
                const snd = (sp._howl as any)?._sounds?.find((x: any) => x._sprite === sid);
                if (snd?._node) {
                ...
                // If playa-core didn't resume, do it ourselves
                if (!sp._isPlaying && sp._isPaused) {
                    sp.resume();
                }
                // Fade in
                if (snd?._node) {
...
async function init(): Promise<void> {
    try {
        ...
        for (const name of MUSIC) {
            ...
            if (!url) { warn(name, "— URL not found"); continue; }
            if (await registerHtml5(name, url, player)) ok++;
        }
