...
ipcMain.handle('run-script', async (event, scriptName) => {
  ...
  return new Promise((resolve) => {
    exec(`npm run ${scriptName}`, { cwd: projectPath, timeout: 300000, maxBuffer: 5 * 1024 * 1024 }, (err, stdout, stderr) => {
...
ipcMain.handle('run-deploy', async (event, scriptName) => {
  ...
  return new Promise((resolve) => {
    exec(`npm run ${name}`, { cwd: projectPath, timeout: 120000, maxBuffer: 5 * 1024 * 1024 }, (err, stdout, stderr) => {
      resolve({ success: !err, output: stdout + '\n' + stderr, error: err ? err.message : null });
    });
  });
});
...
ipcMain.handle('configure-game', async (event, { gameRepoPath }) => {
  ...
  try {
    ...
    const rel = path.relative(projectPath, gameRepoPath);
...
// npm install — one click
ipcMain.handle('npm-install', async () => {
  if (!projectPath) return { error: 'No project open' };
  return new Promise((resolve) => {
    exec('npm install --legacy-peer-deps', { cwd: projectPath, timeout: 240000, maxBuffer: 5 * 1024 * 1024 }, (err, stdout, stderr) => {
      resolve({
        success: !err,
        output: stdout + '\n' + stderr,
